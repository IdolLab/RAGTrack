...
class LasHeR(BaseVideoDataset):
    ...
    def get_sequence_info(self, seq_id):
        ...
        if os.path.isfile(desc_file):
            ...
            if len(lines) != n:
                raise ValueError(
                    f"Number of lines in {desc_file} ({len(lines)}) does not match sequence length {n}."
                )
            text_label = lines
        else:
            text_label = [None] * n
            print(f'Missing description file: {desc_file}')
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        rgb_frame_path = sorted(os.listdir(os.path.join(seq_path,"visible")))
